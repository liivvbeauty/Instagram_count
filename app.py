...
SHEET_ID = os.getenv('LIIVV_SHEET_ID', '1M98FNCa83Y5V9grSlWazoY_xthp_cs32s5xuheBNM0o')
PUBLICITY_SHEET = os.getenv('PUBLICITY_SHEET', 'Publicidades')
CONFIG_SHEET = os.getenv('CONFIG_SHEET', 'Configuracoes')
...
DEFAULT_CONFIG = {
    'BRAND_NAME':'LIIVV BEAUTY','INSTAGRAM_USERNAME':'liivvbeauty',
    'PROFILE_URL':'https://www.instagram.com/liivvbeauty/',
    ...
    'BOOKING_URL':'https://www.instagram.com/liivvbeauty/',
    'SECONDARY_URL':'https://www.instagram.com/liivvbeauty/'
}
...
def instagram_data(username, profile_url):
    try:
        ...
        return {'followers':int(s.get('followers_count',0)),'media_count':int(s.get('media_count',0)),'username':s.get('username',username),'items':items}
    except Exception:
        return {'followers':MOCK_FOLLOWERS_START,'media_count':0,'username':username,'items':[]}
...
def render():
    ...
    data=instagram_data(username,cfg['PROFILE_URL']); prev=read_prev(); delta=0 if prev is None else data['followers']-prev; write_prev(data['followers'])
